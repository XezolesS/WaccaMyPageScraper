...
{
    internal sealed class RateIconsFetcherAction : FetcherAction<string>
    {
        protected override string Url => "https://wacca.marv-games.jp/img/web/music/rate_icon/";

        public RateIconsFetcherAction(Fetcher fetcher) : base(fetcher) { }

        /// <summary>
        /// Fetch all rate icons.
        /// </summary>
                    ...
                    using (var msg = new HttpRequestMessage(HttpMethod.Get, imageUrl))
                    {
                        ...
                        using (var fs = new FileStream(imagePath, FileMode.Create, FileAccess.Write))
                        {
                            ...
                            this._fetcher.Logger?.Information(Localization.Fetcher.DataSaved,
                                $"{Localization.Data.RateIcon}({fileName})", Path.GetFullPath(imagePath));
                        }
                    }
                }
            }
